...
relay_board_i2C_addres = 0x49 
banana_board_i2C_addres = 0x48
...
def banana_board_readAdc(channel):
	...
	bus.write_word_data(banana_board_i2C_addres, DEVICE_REG_CONFIG, swap(config))
	 
	# wait for conversion to complete (blocking)
	while (True):
		
		status=swap(bus.read_word_data(banana_board_i2C_addres,DEVICE_REG_CONFIG))
		
		if (status & CONFIG_OS) != CONFIG_OS_PERFORMING_CONVERSION:
			break

	# read result (note byte swap)
	result=swap(bus.read_word_data(banana_board_i2C_addres,DEVICE_REG_CONVERSION))
	banana_voltages=(result/32768.0)*4.096 #32767?
	# return 16 bit integer A2D result for the specified channel
	return banana_voltages


def role_board_readAdc(channel):
	# sanity check the channel specified
	if ((channel > 3) or (channel < 0)):
		return -1
	...
	#send read command (note byte swap)
	bus.write_word_data(relay_board_i2C_addres, DEVICE_REG_CONFIG, swap(config))
	 
	# wait for conversion to complete (blocking)
	while (True):
		
		status=swap(bus.read_word_data(relay_board_i2C_addres,DEVICE_REG_CONFIG))
		
		if (status & CONFIG_OS) != CONFIG_OS_PERFORMING_CONVERSION:
			break

	# read result (note byte swap)
	result=swap(bus.read_word_data(relay_board_i2C_addres,DEVICE_REG_CONVERSION))
	banana_voltages=(result/32768.0)*4.096 #32767?
	# return 16 bit integer A2D result for the specified channel
	return banana_voltages
...
if __name__ == '__main__':
	...
	try:
		while True:
			...
			s2=" Voltages 0x49 "
			for i in range(0,4):
				adc0x48_input_voltage = banana_board_readAdc(i)
				board0x48_input_voltage = adc0x48_input_voltage*3.024379
				s1+=str(i)+": %04f "% board0x48_input_voltage+"  "
			print (s1)
				
			for i in range(0,4):
				adc0x49_input_voltage = role_board_readAdc(i)
				board0x49_input_voltage = adc0x49_input_voltage*3.024379
				s2+=str(i)+": %04f "% board0x49_input_voltage +"  "
			print (s2)
			time.sleep(1/2)
