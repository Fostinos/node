...
static uint8_t LoRaWAN_DataUp(MACPayload_t * packet, uint8_t* buffer, uint8_t bufferSize, bool isConfirmed)
{
	...
	// FOpts
	if(bufferSize < index + packet->FHDR.FCtrl.uplink.FOptsLen)
	{
		return 0;
	}
    memcpy(buffer + index, packet->FHDR.FOpts, packet->FHDR.FCtrl.uplink.FOptsLen);
	index += packet->FHDR.FCtrl.uplink.FOptsLen;
